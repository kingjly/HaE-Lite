...
项目内置一套默认规则示例，位于 `Rules.yml`，涵盖常见敏感信息：
...
HaE-Lite/
├── manifest.json              # 扩展清单
├── background.js              # 后台入口（MV3 Service Worker）
├── background/                # 后台模块化代码
│   ├── requestProcessor.js    # 请求过滤与域名匹配
│   └── captureInit.js         # 调试器附加/分离与事件注册
├── devtools/                  # DevTools 面板
│   ├── devtools.html          # 面板入口
│   ├── devtools.js            # 面板脚本
│   ├── panel.html             # 主界面
│   ├── panel.js               # 面板逻辑
│   ├── networkCapture.js      # 请求/响应抓取桥接
│   ├── styles.css             # 样式
│   └── views/                 # 视图模块
│       ├── requestsView.js    # 请求列表与高亮
│       ├── rulesView.js       # 规则管理
│       ├── uiControls.js      # UI 控件与交互
│       ├── settingsView.js    # 设置视图
│       ├── settingsInit.js    # 设置初始化
│       └── previewInit.js     # 预览初始化
├── shared/                    # 共享模块
│   ├── storage.js             # 存储封装
│   ├── ruleEngine.js          # 规则引擎
│   └── utils.js               # 工具函数
└── Rules.yml                  # 默认规则文件（可在面板导入）
```
...
- 事件驱动的后台 Service Worker（MV3）：当全局开关启用时会自动附加到 HTTP(S) 标签页进行捕获；不是持久后台页
- 无云同步功能，数据完全本地存储
